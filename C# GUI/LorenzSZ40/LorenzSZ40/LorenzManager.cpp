...
void LorenzManager::ConsoleMenu()
{
        ...
        if ( cin >> choice )
        {
            ...
            else if ( choice == 9) { SaveConfigConsole(); }													// 9 to save a full configuration of current data.
            else if ( choice == 10) { LoadConfigConsole(); }													// 10 to load in a full configuration.
...
// Randomises the current default PIN positions.
void LorenzManager::RandomisePIN()
{
				wheelcollection.Ψ1.setDefaultWheelPosition( rand() % 43 + 1 );
                wheelcollection.Ψ2.setDefaultWheelPosition( rand() % 47 + 1 );
                wheelcollection.Ψ3.setDefaultWheelPosition( rand() % 51 + 1 );
...
void LorenzManager::SaveConfig(string configuration)
{

}

// Displays the screen to save a full config of the machine.
void LorenzManager::SaveConfigConsole()
{
...
void LorenzManager::LoadConfig(string configFile)
{
                ...
                if ( config.is_open() )
                {
                    ...
                    for ( int i = 0; i < 13; i++ )
                    {
                        if ( i != 0 )
                        {
                            wheelvals[i] = atoi( fromfile[i].c_str() );
                        }
                    }
...
void LorenzManager::LoadConfigConsole()
{
            PrintTitle();
            char load;
